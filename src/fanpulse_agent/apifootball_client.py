...
class APIFootballClient:
    def __init__(
        ...
        base_url: str = APIFOOTBALL_BASE_URL,
        timeout: float = 8.0,
    ):
        self.api_key = api_key or os.environ.get("APIFOOTBALL_API_KEY") or ""
    ...
    def next_team_fixtures(self, team_id: int) -> list[dict[str, Any]]:
        payload = self._get("fixtures", {"team": str(team_id), "next": "5"})
    ...
    def next_league_fixtures(self, league_id: int, season: Optional[int] = None) -> list[dict[str, Any]]:
        payload = self._get(
            "fixtures",
            {
